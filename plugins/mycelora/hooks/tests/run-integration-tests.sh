#!/usr/bin/env bash
# Script d'integration pour les hooks mycelora.
...
mycelora_secure_curl() {
  local body_file="$1"
  local cfgfile resp_file http_code curl_rc
  cfgfile="$(mktemp /tmp/mycelora-integ-cfg.XXXXXX)"
...
CFGEOF
  resp_file="$(mktemp /tmp/mycelora-integ-resp.XXXXXX)"
